...
"High-quality" here is the multi-dimensional sense from the data-quality literature — *fitness for use* (Juran 1974; Wang & Strong 1996) across the dimensions of availability, usability, reliability, relevance, and presentation (Cai & Zhu 2015). See [`data-modeling.md#data-quality-dimensions`](references/data-modeling.md#data-quality-dimensions) for the full framework and how each pipeline phase contributes to which dimension.

The skill takes the civic-data tradition's framing seriously while keeping its limits in view. Open data alone does not produce information justice (Johnson 2014); civic hacking's history is older and more politically contested than the open-data era suggests (Schrock 2016); "data culture" inside an institution is a field of contested meanings, not a coherent organizational asset (Casemajor 2025). See [`movement-history.md#critical-perspectives-worth-absorbing`](references/movement-history.md#critical-perspectives-worth-absorbing). The skill produces the artifacts; the politics of who benefits from the artifacts is the maintainer's responsibility to think about explicitly, in the README and AGENTS.md, project by project.
...
- **Documentation conventions and quality dimensions.** Data dictionaries, harmonization crosswalks, per-extract provenance, filter-and-pivot recipes for tidy ↔ wide, plus the Cai/Zhu five-dimension quality framework and the profiling / measurement / monitoring decomposition (Ehrlinger & Wöß 2022). See [`data-modeling.md`](references/data-modeling.md).
- **Auditing and bulletproofing patterns.** Discovery of new upstream sources, reconciliation of processed data against original totals, and a pre-extraction bulletproofing checklist that maps the ProPublica practitioner tradition onto the academic data-quality assessment literature (Batini et al. 2009). See [`discovery-and-audit.md`](references/discovery-and-audit.md).
- **Movement and methodological history.** The civic data liberation tradition (Sunlight Foundation, PDF Liberation Working Group, MuckRock, PUDL, BoulderPublicData) plus the scholarly read from outside (Schrock on civic-hacking history, Baack on open-source modulation, Johnson on information justice, Casemajor on contested data cultures), the academic counterparts (CRISP-DM, Shigarov's table-understanding taxonomy, Holstein's data-understanding dimensions), and the data-quality lineage (Juran → Wang & Strong → Batini → Cai). Read [`movement-history.md`](references/movement-history.md) once at the start of a new project so the framing is shared.
