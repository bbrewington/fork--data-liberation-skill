# Toolchain: PDF Extraction

When the user has a PDF and wants data out of it, this reference is the decision tree. The tools — `pdfplumber`, `camelot`, `tesseract` via `pytesseract` — each excel at a different class of PDF, and most real liberation projects use two or three of them in combination.

## First: identify the PDF type

Before opening any extractor, classify the input. Run this short triage and write the result in the project's Survey notes.
...
A quick sanity check: open the PDF in a viewer and try to **select and copy text** from a table cell. If the copied text is the cell content, the text layer is good and you want `pdfplumber` or `camelot`. If you get nothing or garbled glyphs, it's scanned and you need OCR.
...
The configuration vocabulary is documented in [the pdfplumber README](https://github.com/jsvine/pdfplumber#extracting-tables) and walked through in [Unstract's pdfplumber guide](https://unstract.com/blog/guide-to-pdfplumber-text-and-table-extraction-capabilities/). Two patterns worth memorizing:
...
When the PDF has no text layer (or a corrupt one), the path is: rasterize each page, OCR it with [Tesseract](https://github.com/tesseract-ocr/tesseract), then handle the OCR output as text. Quality varies dramatically by document and is sensitive to preprocessing.
